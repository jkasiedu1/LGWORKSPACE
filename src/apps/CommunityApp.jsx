    ...
    <>
      ...
      <div className="border-t border-stone-100 flex">
        <div className="relative flex-1" ref={pickerRef}>
          <button
            onClick={() => myReaction ? onReact(myReaction.type) : setPickerOpen(v => !v)}
            className={`w-full flex justify-center items-center gap-1.5 py-3 text-xs font-semibold rounded-none transition-colors
              ${myReaction ? 'text-teal-600 bg-teal-50/50' : 'text-stone-500 hover:bg-stone-50 active:bg-stone-100'}`}
          >
            <span className="text-base">{myReaction ? myReaction.emoji : '👍'}</span>
            <span>{myReaction ? myReaction.label : 'Like'}</span>
          </button>
          {pickerOpen && (
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 flex gap-1 bg-white border border-stone-200 rounded-full shadow-xl px-3 py-2 z-30">
              {REACTIONS.map(r => (
                <button
                  key={r.type}
                  title={r.label}
                  onPointerDown={(e) => { e.preventDefault(); onReact(r.type); setPickerOpen(false); }}
                  className="text-2xl hover:scale-125 active:scale-110 transition-transform px-1.5 py-0.5"
                >
                  {r.emoji}
                </button>
              ))}
        ...
        <button
          onClick={onOpenPost}
          className="flex-1 flex justify-center items-center gap-1.5 py-3 text-stone-500 hover:bg-stone-50 active:bg-stone-100 text-xs font-semibold transition-colors"
        >
        ...
        <div className="px-4 pb-4">
          ...
          <div className="flex gap-2 justify-end mt-2">
            ...
            <button onClick={() => { onEdit(post.id, editContent); setEditing(false); }}
              className="px-3 py-1.5 text-xs font-semibold text-white rounded-lg bg-stone-900">Save</button>
          </div>
        </div>
      ) : post.content ? (
  ...
  return (
    ...
    <div className="animate-in fade-in duration-500 text-left h-full">
      <div className="flex justify-between items-center mb-4">
        <div>
          <h1 className="font-serif text-2xl sm:text-3xl font-bold text-stone-900 tracking-tight">Church Community</h1>
          <p className="text-stone-500 text-xs sm:text-sm mt-0.5">Connect, share updates, and message members directly.</p>
        </div>
        {/* Mobile DM button in header */}
        <button
          onClick={() => setShowDmSheet(true)}
          className="lg:hidden flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold text-white shadow bg-stone-900"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
        ...
        <div className="lg:col-span-2 space-y-4 order-1 lg:order-2">
          ...
          <div className={`bg-white rounded-2xl border shadow-sm transition-all ${isPostFocused ? 'border-stone-300 shadow-md' : 'border-stone-200'}`}>
            <div className="p-3 flex gap-2.5">
              <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full shrink-0 flex items-center justify-center text-xs sm:text-sm font-bold text-white ${avatarColor(displayName)}`}>{myInitials}</div>
              <textarea
            ...
            {isPostFocused && (
              <div className="border-t border-stone-100">
                <div className="px-3 pt-2 pb-1 flex items-center gap-1 flex-wrap bg-stone-50 border-t border-stone-200">
                  <input ref={photoInputRef} type="file" accept="image/*,video/*" className="hidden" onChange={handlePhotoSelected}/>
                  <button onClick={() => photoInputRef.current?.click()}
                    className="flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs font-semibold text-stone-500 hover:bg-stone-100 active:bg-stone-200 transition-colors">
                    <ImageIcon size={15} className="text-emerald-500"/> <span className="hidden xs:inline">Photo/Video</span>
                  </button>
                  <button className="flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs font-semibold text-stone-500 hover:bg-stone-100 active:bg-stone-200">
                    <LinkIcon size={15} className="text-sky-500"/> <span className="hidden xs:inline">Link</span>
                  </button>
                  <button className="flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs font-semibold text-stone-500 hover:bg-stone-100 active:bg-stone-200">
                    <Smile size={15} className="text-amber-500"/> <span className="hidden xs:inline">Emoji</span>
                  </button>
                </div>
                <div className="px-3 pb-2 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={handlePost} disabled={!newPostContent.trim()} className="px-4 py-1.5 rounded-lg text-xs font-semibold text-white disabled:opacity-50 transition-opacity bg-stone-900">Post</button>
                  </div>
                </div>
              </div>
            )}
        ...
        {/* RIGHT — DM (desktop only) */}
        <div className="hidden lg:block lg:col-span-1 order-2 lg:order-3">
          <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden flex flex-col lg:h-[620px] lg:sticky lg:top-20 shadow-sm">
      ...
      <div className="fixed inset-0 z-[70] lg:hidden" style={{background:'rgba(0,0,0,0.5)'}} onClick={() => setShowDmSheet(false)}>
        <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl shadow-2xl flex flex-col" style={{maxHeight:'85dvh'}} onClick={e => e.stopPropagation()}>
          ...
          <div className={`px-4 py-3 flex justify-between items-center border-b border-stone-100`}>
            ...
            <button onClick={() => setShowDmSheet(false)} className="text-stone-400 hover:text-stone-600"><X size={20}/></button>
          </div>
