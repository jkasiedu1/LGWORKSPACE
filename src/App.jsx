    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-stone-500 text-sm mt-1">Live from Firebase</p>
        </div>
        ...
        <div className="fixed inset-0 bg-stone-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2 mt-6">
                <button onClick={() => setIsAdding(false)} className="px-4 py-2 text-stone-600 hover:bg-stone-100 rounded-md font-medium text-sm">Cancel</button>
                <button onClick={handleAddEvent} disabled={isSaving} className={`px-4 py-2 ${theme.bg} text-white rounded-md font-medium text-sm hover:opacity-90 flex items-center gap-2`}>
                  {isSaving ? <Loader2 size={16} className="animate-spin" /> : 'Save Event'}
                </button>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white border border-stone-200 rounded-xl shadow-sm overflow-hidden p-6">
        <h3 className="font-semibold text-stone-800 mb-4">Upcoming Events</h3>
        <div className="divide-y divide-stone-100">
          {events.length === 0 && <p className="text-stone-500 text-sm py-4">No events scheduled. Add one above.</p>}
          {events.map((event) => (
            <div key={event.id} className="py-4 flex items-center justify-between group">
              <div>
                ...
                <p className="text-sm text-stone-500">{event.date} at {event.time}</p>
              </div>
              <button onClick={() => handleDelete(event.id)} className="text-stone-400 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity">
                Delete
              </button>
            </div>
          ))}
...
// ... Rest of the original static modules ...
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-stone-500 text-sm mt-1">Your data is now live from Firebase.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <HomeMetricCard title="Upcoming Services" value={events.length} label="Live Events" color="text-amber-600" />
...
function ServicesApp({ theme, planItems, setPlanItems }) { return <div className="p-8 text-center text-stone-500 border border-dashed border-stone-300 rounded-xl bg-stone-50/50">Services App (Static View)</div>; }
function MusicApp({ theme }) { return <div className="p-8 text-center text-stone-500 border border-dashed border-stone-300 rounded-xl bg-stone-50/50">Music Library (Static View)</div>; }
function TeamsApp({ theme, setActiveApp }) { return <div className="p-8 text-center text-stone-500 border border-dashed border-stone-300 rounded-xl bg-stone-50/50">Team Portals (Static View)</div>; }
function GivingApp({ theme }) { return <div className="p-8 text-center text-stone-500 border border-dashed border-stone-300 rounded-xl bg-stone-50/50">Giving Dashboard (Static View)</div>; }
function WorkflowsApp({ theme }) { return <div className="p-8 text-center text-stone-500 border border-dashed border-stone-300 rounded-xl bg-stone-50/50">Workflows (Static View)</div>; }
function SecurityApp({ theme }) { return <div className="p-8 text-center text-stone-500 border border-dashed border-stone-300 rounded-xl bg-stone-50/50">Security (Static View)</div>; }
function ReportingApp({ theme }) { return <div className="p-8 text-center text-stone-500 border border-dashed border-stone-300 rounded-xl bg-stone-50/50">Reporting (Static View)</div>; }
