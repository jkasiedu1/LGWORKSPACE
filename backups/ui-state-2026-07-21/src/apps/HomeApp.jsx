...
export default function HomeApp({ events, people, isAdmin, isSeniorPastor, setActiveApp, loadingPeople, loadingEvents, user }) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 text-left">
      <div className="rounded-3xl border border-white/70 bg-gradient-to-r from-white/95 via-orange-50/70 to-sky-50/70 shadow-xl p-5 sm:p-7 relative overflow-hidden">
        <div className="absolute -right-12 -top-14 h-32 w-32 rounded-full bg-orange-300/35 blur-2xl" />
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
          <div>
            <p className="text-[11px] uppercase tracking-[0.2em] text-stone-500 font-bold mb-1">Executive Briefing</p>
            <h1 className="font-serif text-3xl sm:text-5xl font-bold text-stone-900 tracking-tight leading-[1.08]">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-orange-600 to-amber-500 text-white px-5 py-2.5 text-sm font-bold hover:from-orange-500 hover:to-amber-400 transition-all shadow-lg"
          >
            Open Calendar
        ...
        <div className="xl:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2 rounded-2xl border border-white/80 bg-white/80 shadow-sm p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-stone-500">Next Event</p>
              ...
              <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-stone-900 to-stone-700 text-white flex items-center justify-center shadow-lg">
          ...
          <div className="md:col-span-2 rounded-2xl border border-white/80 bg-white/80 shadow-sm p-5">
            ...
            <div className="h-2 rounded-full bg-stone-200 overflow-hidden">
              <div
                className="h-full rounded-full bg-gradient-to-r from-orange-500 via-amber-400 to-teal-400 transition-all duration-700"
          ...
          <div className="md:col-span-2 rounded-2xl border border-white/80 bg-white/80 shadow-sm p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[10px] uppercase tracking-[0.16em] font-bold text-stone-500">Role Adaptive</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {missionLanes.map((lane) => (
                <button
                  ...
                  className="text-left p-3 rounded-xl border border-stone-200 bg-gradient-to-br from-white to-stone-50 hover:from-white hover:to-white transition-all hover:-translate-y-0.5"
                >
                  <p className="text-[10px] uppercase tracking-[0.16em] text-stone-500 font-bold">{lane.title}</p>
            ...
            <div className="space-y-2">
              {events.slice(0, 3).map((event) => (
                <button
                  key={event.id}
                  onClick={() => setActiveApp('calendar')}
                  className="w-full text-left p-3 rounded-lg border border-stone-200 hover:border-stone-300 hover:bg-white transition-colors"
                >
