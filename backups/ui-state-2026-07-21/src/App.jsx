...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans relative overflow-hidden">
      ...
      <div className="absolute -top-24 -left-24 w-96 h-96 rounded-full ambient-blob" style={{ backgroundColor: 'var(--blob-a)' }} />
      <div className="absolute top-24 -right-20 w-80 h-80 rounded-full ambient-blob" style={{ backgroundColor: 'var(--blob-b)', animationDelay: '1.2s' }} />
      {toastMsg && (
        <div className="fixed bottom-[88px] lg:bottom-6 left-1/2 -translate-x-1/2 z-[55] bg-gradient-to-r from-stone-900 to-stone-700 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 animate-in slide-in-from-bottom-4 fade-in">
      ...
      {showUsernameOnboarding && (
        <div className="fixed inset-0 bg-stone-900/65 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-white/95 backdrop-blur-xl rounded-2xl border border-stone-200 shadow-2xl p-6">
      ...
      <header className="glass-surface fixed top-0 left-2 right-2 sm:left-4 sm:right-4 mt-2 z-50 shadow-sm rounded-2xl">
        <div className="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
            ...
            <div className="relative">
              ...
              >
                ...
                <div className="flex flex-col items-start leading-none mt-1">
                  <div className="flex items-center gap-1.5">
                    <span className="text-stone-900 font-serif font-bold text-xl leading-none">Lifegate AG</span>
                  ...
                  <span className="text-[10px] font-sans font-bold uppercase tracking-[0.17em] text-stone-400 mt-0.5">{theme.name}</span>
                </div>
              ...
              {isAppSwitcherOpen && (
                <>
                  ...
                  <div className="absolute top-full left-0 mt-3 w-80 bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl border border-stone-200 p-3 grid grid-cols-3 gap-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                    {visibleApps.map((app) => {
                      const Icon = app.icon;
                      return (
                        <button
                          ...
                          className={`flex flex-col items-center justify-center gap-2 p-3 rounded-xl hover:bg-stone-50 transition-colors ${activeApp === app.id ? 'bg-stone-50 ring-1 ring-stone-200' : ''}`}
                        >
          ...
          <div className="hidden md:flex flex-1 max-w-xl mx-8">
            <form onSubmit={handleGlobalSearch} className="relative w-full group flex">
              <input
                ...
                className="w-full pl-4 pr-10 py-2 bg-white/70 border border-transparent rounded-l-xl text-sm focus:bg-white focus:border-orange-300 focus:ring-2 focus:ring-orange-100 transition-all outline-none"
              />
              <button type="submit" className="bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-400 hover:to-amber-400 text-white px-4 rounded-r-xl transition-colors flex items-center justify-center">
                <Search size={16} />
              </button>
              {globalSearchInput && (
                <button
          ...
          <div className="flex items-center gap-4">
              <button
                ...
                className="hidden lg:flex items-center gap-2 rounded-full bg-white/75 border border-white/60 px-3 py-1.5 text-xs font-semibold text-stone-700 hover:bg-white transition-colors"
                title="Toggle theme"
              >
            ...
            <div className="relative" ref={notifRef}>
              ...
              {isNotifOpen && (
                <div className="fixed sm:absolute top-16 sm:top-10 left-2 right-2 sm:left-auto sm:right-0 sm:w-80 bg-white/95 backdrop-blur-xl border border-stone-200 rounded-2xl shadow-xl z-[60] overflow-hidden">
      ...
      <main id="main-content" className="max-w-[1680px] w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-28 lg:pb-6 flex-1 relative z-10 pt-[108px] lg:pt-24" role="main">
        <div className="grid grid-cols-1 xl:grid-cols-[84px_minmax(0,1fr)_320px] gap-5">
          <aside className="hidden xl:flex flex-col glass-surface rounded-3xl p-3 h-[calc(100vh-8rem)] sticky top-24">
            {visibleApps.slice(0, 8).map((app) => {
              ...
              return (
                <button
                  ...
                  className={`h-12 w-12 mx-auto mt-2 rounded-xl flex items-center justify-center transition-colors ${activeApp === app.id ? 'bg-gradient-to-br from-stone-900 to-stone-700 text-white shadow-lg' : 'text-stone-500 hover:bg-white/70'}`}
                  title={app.name}
                >
                  <Icon size={18} />
                </button>
          ...
          <aside className="hidden xl:flex flex-col glass-surface rounded-3xl p-4 h-[calc(100vh-8rem)] sticky top-24">
            ...
            <div className="mt-auto rounded-xl bg-gradient-to-br from-stone-900 to-stone-700 text-white p-3">
          ...
          {mobileDockApps.map((app) => {
            ...
            return (
              <button
                key={`dock-${app.id}`}
                onClick={() => setActiveApp(app.id)}
                className={`h-12 rounded-xl flex flex-col items-center justify-center gap-0.5 ${activeApp === app.id ? 'bg-gradient-to-br from-stone-900 to-stone-700 text-white' : 'text-stone-600 hover:bg-white/70'}`}
              >
                <Icon size={15} />
